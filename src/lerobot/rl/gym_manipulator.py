...
from lerobot.robots import (  # noqa: F401
    RobotConfig, make_robot_from_config, so100_follower, jakaS12,
)
...
class RobotEnv(gym.Env):
    ...
    def _get_observation(self) -> dict[str, Any]:
        ...
        raw_joint_joint_position = {
            f"{name}.pos": obs_dict[f"{name}._joint_position"]
            for name in self._joint_names
        }
        joint_positions = np.array([
            raw_joint_joint_position[f"{name}.pos"]
            for name in self._joint_names
        ])
    ...
    def reset(
        ...
        *,
        ...
        self._raw_joint_positions = {
            f"{key}.pos": obs[f"{key}.pos"]
            for key in self._joint_names
        }
    ...
    def render(self) -> None:
        ...
        if current_observation is not None:
            ...
            for key in image_keys:
                cv2.imshow(
                    key,
                    cv2.cvtColor(current_observation[key].numpy(),
                                 cv2.COLOR_RGB2BGR))
                cv2.waitKey(1)
...
def make_robot_env(cfg: HILSerlRobotEnvConfig) -> tuple[gym.Env, Any]:
    ...
    display_cameras = (cfg.processor.observation.display_cameras
                       if cfg.processor.observation is not None else False)
...
def make_processors(
    ...
    device: str = "cpu"
) -> tuple[DataProcessorPipeline[EnvTransition, EnvTransition],
    ...
    """
    terminate_on_success = (cfg.processor.reset.terminate_on_success
                            if cfg.processor.reset is not None else True)
    ...
    if cfg.name == "gym_hil":
        ...
        return DataProcessorPipeline(
            ...
            to_output=identity_transition), DataProcessorPipeline(
    ...
    if cfg.processor.inverse_kinematics is not None:
        kinematics_solver = RobotKinematics(
            ...
            target_frame_name=cfg.processor.inverse_kinematics.
            target_frame_name,
    ...
    if cfg.processor.observation is not None:
        if cfg.processor.observation.add_joint_velocity_to_observation:
            env_pipeline_steps.append(
                JointVelocityProcessorStep(dt=1.0 / cfg.fps))
        if cfg.processor.observation.add_current_to_observation:
            env_pipeline_steps.append(
                MotorCurrentProcessorStep(robot=env.robot))
    ...
    if cfg.processor.reset is not None:
        env_pipeline_steps.append(
            TimeLimitProcessorStep(
                max_episode_steps=int(cfg.processor.reset.control_time_s *
                                      cfg.fps)))
    ...
    if (cfg.processor.reward_classifier is not None
        ...
        env_pipeline_steps.append(
            RewardClassifierProcessorStep(
                pretrained_path=cfg.processor.reward_classifier.
                pretrained_path,
                device=device,
                success_threshold=cfg.processor.reward_classifier.
                success_threshold,
    ...
    action_pipeline_steps = [
        ...
        InterventionActionProcessorStep(
            ...
            if cfg.processor.gripper is not None else False,
            terminate_on_success=terminate_on_success,
        ),
    ]
    ...
    if cfg.processor.inverse_kinematics is not None and kinematics_solver is not None:
        ...
        inverse_kinematics_steps = [
            MapTensorToDeltaActionDictStep(
                use_gripper=cfg.processor.gripper.use_gripper if cfg.processor.
                gripper is not None else False),
            MapDeltaActionToRobotActionStep(),
            EEReferenceAndDelta(
                kinematics=kinematics_solver,
                end_effector_step_sizes=cfg.processor.inverse_kinematics.
                end_effector_step_sizes,
                motor_names=motor_names,
                use_latched_reference=False,
                use_ik_solution=True,
            ),
            EEBoundsAndSafety(end_effector_bounds=cfg.processor.
                              inverse_kinematics.end_effector_bounds, ),
            GripperVelocityToJoint(
                clip_max=cfg.processor.max_gripper_pos,
                speed_factor=1.0,
                discrete_gripper=True,
            ),
            InverseKinematicsRLStep(kinematics=kinematics_solver,
                                    motor_names=motor_names,
...
def step_env_and_process_transition(
    ...
    transition[TransitionKey.OBSERVATION] = (
        ...
        if hasattr(env, "get_raw_joint_positions") else {})
    processed_action_transition = action_processor(transition)
    processed_action = processed_action_transition[TransitionKey.ACTION]

    if processed_action.ndim == 2 and processed_action.shape[0] == 1:
    ...
    truncated = truncated or processed_action_transition[
...
def replay_trajectory(env: gym.Env, action_processor: DataProcessorPipeline,
                      cfg: GymManipulatorConfig) -> None:
    ...
    episode_frames = dataset.hf_dataset.filter(
        lambda x: x["episode_index"] == cfg.dataset.replay_episode)
    ...
    for action_data in actions:
        ...
        transition = create_transition(
            observation=env.get_raw_joint_positions() if hasattr(
                env, "get_raw_joint_positions") else {},
...
def control_loop_for_binary_classifier(
    env: gym.Env,
    env_processor: DataProcessorPipeline[EnvTransition, EnvTransition],
    action_processor: DataProcessorPipeline[EnvTransition, EnvTransition],
    teleop_device: Teleoperator,
            ...
            if cfg.mode == "record":
                ...
                frame = {
                    **observations,
                    ...
                    action_to_record.cpu(),
                    REWARD:
                    np.array([transition[TransitionKey.REWARD]],
                             dtype=np.float32),
                    DONE:
                    np.array([terminated or truncated], dtype=bool),
                }
